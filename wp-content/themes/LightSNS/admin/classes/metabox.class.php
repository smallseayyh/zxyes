...
if( ! class_exists( 'LightSNS_Metabox' ) ) {
class LightSNS_Metabox {
...
$classes[] = 'jinsom-panel-post-formats';
...
$classes[] = 'jinsom-panel-post-format-'. $format;
}

if( ! in_array( $saved_post_format, $this->post_formats ) ) {
$classes[] = 'jinsom-panel-hide';
} else {
$classes[] = 'jinsom-panel-show';
}
...
$classes[] = 'jinsom-panel-page-templates';

foreach( $this->page_templates as $template ) {
$classes[] = 'jinsom-panel-page-'. preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
}
...
echo '<div class="csf jinsom-panel-theme-'. $this->args['theme'] .' jinsom-panel-metabox">';

echo '<div class="jinsom-panel-wrapper'. $show_all .'">';

if( $has_nav ) {

echo '<div class="jinsom-panel-nav jinsom-panel-nav-metabox" data-unique="'. $this->unique .'">';

echo '<ul>';
$tab_key = 1;
foreach( $this->sections as $section ) {

$tab_error = ( ! empty( $errors['sections'][$tab_key] ) ) ? '<i class="jinsom-panel-label-error jinsom-panel-error">!</i>' : '';
$tab_icon = ( ! empty( $section['icon'] ) ) ? '<i class="jinsom-panel-icon '. $section['icon'] .'"></i>' : '';
...
echo '<div id="jinsom-panel-section-'. $this->unique .'_'. $section_key .'" class="jinsom-panel-section'. $onload .'">';

$section_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="jinsom-panel-icon '. $section['icon'] .'"></i>' : '';
$section_title = ( ! empty( $section['title'] ) ) ? $section['title'] : '';

echo ( $section_title || $section_icon ) ? '<div class="jinsom-panel-section-title"><h3>'. $section_icon . $section_title .'</h3></div>' : '';
...
LightSNS::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );

}

} else {

echo '<div class="jinsom-panel-no-option jinsom-panel-text-muted">没有数据</div>';
...
echo '<div class=" jinsom-panel-metabox-restore">';
echo '<label>';
echo '<input type="checkbox" name="'. $this->unique .'[_restore]" />';
echo '<span class="button jinsom-panel-button-restore">重置</span>';
echo '</label>';
echo '</div>';
...
echo ( $has_nav ) ? '<div class="jinsom-panel-nav-background"></div>' : '';
