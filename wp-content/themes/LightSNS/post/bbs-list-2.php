...
$bbs_content=strip_tags(get_the_content());
$bbs_content = preg_replace("/\[file[^]]+\]/", "[附件]", $bbs_content);
$bbs_content = preg_replace("/\[video[^]]+\]/", "[视频]", $bbs_content);
$bbs_content = preg_replace("/\[music[^]]+\]/", "[音乐]", $bbs_content);
$bbs_content=preg_replace('/\s(?=\s)/','',$bbs_content);
$title_color=get_post_meta($post_id,'title_color',true);//标题颜色
?>
<div class="jinsom-bbs-list-1 jinsom-bbs-list-2" id="jinsom-bbs-post-<?php echo $post_id;?>">
...
<div class="right">
<!-- 标题 -->
<h2><a href="<?php the_permalink(); ?>" <?php if($post_target){echo 'target="_blank"';}?> <?php if($title_color){echo 'style="color:'.$title_color.'"';}?>>
<?php 
...
<?php echo convert_smilies(mb_substr($bbs_content,0,90,'utf-8')).'...';?>
</div>
<div class="jinsom-bbs-list-2-img-list">
<?php echo jinsom_get_single_img(get_the_content(),$post_id);?>	
</div>
