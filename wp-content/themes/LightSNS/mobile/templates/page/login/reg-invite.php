...
?>
<div data-page="reg-invite" class="page no-tabbar">
...
<div class="center sliding"><?php _e('邀请码注册','jinsom');?></div>
<div class="right">
...
<div class="jinsom-login-input-form jinsom-reg-invite">
<div class="jinsom-mobile-input">
<p class="name"><input type="text" placeholder="<?php _e('账号','jinsom');?>"></p>
<p class="code"><input type="text" placeholder="<?php _e('邀请码','jinsom');?>">
<a href="<?php echo do_shortcode(jinsom_get_option('jinsom_invite_code_get_url'));?>" class="link"><?php _e('获取邀请码','jinsom');?></a>
</p>
...
<div class="jinsom-login-btn" id="reg-3"><?php _e('注册','jinsom');?></div>
<script type="text/javascript">
new TencentCaptcha(document.getElementById('reg-3'),'<?php echo $jinsom_machine_verify_appid;?>',function(res){
if(res.ret === 0){jinsom_pop_reg_invite(res.ticket,res.randstr);}
});
</script>
<?php }else{?>
<div class="jinsom-login-btn" onclick="jinsom_pop_reg_invite('','')"><?php _e('注册','jinsom');?></div>
<?php }?>
</div>
