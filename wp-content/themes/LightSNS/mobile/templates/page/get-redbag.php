...
if($redbag_type=='follow'&&!jinsom_is_follow_author($author_id,$user_id)){
$redbag_text='<m>'.__('你需要关注作者才可以领取该红包！','jinsom').'</m>';
}else{
...
$redbag_text='<m>'.__('领取失败！你已经被对方加入黑名单','jinsom').'</m>';
}
}

?>
<div data-page="get-redbag" class="page no-tabbar">
...
<div class="center sliding"><?php _e('红包详情','jinsom');?></div>
<div class="right">
...
echo '<li>'.$nice.'<div class="avatarimg"><a href="'.jinsom_mobile_author_url($data->user_id).'" class="link">'.jinsom_avatar($data->user_id,'40',avatar_type($data->user_id)).jinsom_verify($data->user_id).'</a></div><div class="info"><div class="name">'.jinsom_nickname($data->user_id).'</div><div class="time">'.$data->time.'</div></div><div class="credit">'.$data->credit.$credit_name.'</div></li>';
}
?>
</div>
</div>
