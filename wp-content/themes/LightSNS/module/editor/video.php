...
$publish_topic_on_off = jinsom_get_option('jinsom_publish_video_topic_on_off');
$publish_add_topic_max = jinsom_get_option('jinsom_publish_video_add_topic_max');
$topic=$_POST['topic'];
...
$title_max= jinsom_get_option('jinsom_publish_video_title_max_words');
$content_max= jinsom_get_option('jinsom_publish_video_cnt_max_words');
...
if(!isset($_POST['video-img-url'])){
$data_arr['code']=0;
$data_arr['msg']='请输入视频封面地址或上传视频封面！';
header('content-type:application/json');
echo json_encode($data_arr);
exit();		
}
...
update_post_meta($post_id,'post_type','video');//类型
update_post_meta($post_id, 'video_url',htmlentities($_POST['video-url'],ENT_QUOTES,'UTF-8'));//视频地址
update_post_meta($post_id, 'video_img',htmlentities($_POST['video-img-url'],ENT_QUOTES,'UTF-8'));//视频封面
update_post_meta($post_id, 'video_time',(int)$_POST['video-time']);//视频时长

$video_lists=$_POST['video_lists'];
if(jinsom_is_admin($user_id)&&$video_lists!=''){
$video_lists_arr=explode(",",$video_lists);
foreach ($video_lists_arr as $data) {
$data_arr=explode("|",$data);
update_post_meta($data_arr[1],'video_lists',$video_lists);//更新视频集数
}
if(!strpos($video_lists,'|'.$post_id.',')){
delete_post_meta($post_id,'video_lists');
}
}
