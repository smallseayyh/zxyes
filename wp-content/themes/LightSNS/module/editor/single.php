...
$publish_topic_on_off = jinsom_get_option('jinsom_publish_single_topic_on_off');
$publish_add_topic_max = jinsom_get_option('jinsom_publish_single_add_topic_max');
$topic=$_POST['topic'];
...
$title_max= jinsom_get_option('jinsom_publish_single_title_max_words');
$content_max= jinsom_get_option('jinsom_publish_single_cnt_max_words');


//售价范围
$price_mini = jinsom_get_option('jinsom_publish_price_mini');
...
$jinsom_single_hide_words_max=jinsom_get_option('jinsom_single_hide_words_max');
if(!$jinsom_single_hide_words_max){$jinsom_single_hide_words_max=10000;}
if($hide_content_number>$jinsom_single_hide_words_max){
$data_arr['code']=0;
$data_arr['msg']='隐藏内容不能超过'.$jinsom_single_hide_words_max.'字！';
header('content-type:application/json');
echo json_encode($data_arr);
exit();		
}
...
// update_post_meta($post_id,'post_type','single');//类型
...
if($power==1||$power==2||$power==4||$power==5||$power==6||$power==7||$power==8){
if($power==1){
update_post_meta($post_id,'post_price',$_POST['price']);//更新售价
}else{
