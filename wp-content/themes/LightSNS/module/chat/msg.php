...
//判断是否开启免打扰模式
$im_privacy=get_user_meta($author_id,'im_privacy',true);
if($im_privacy){
if(!jinsom_is_follow_author($user_id,$author_id)&&!jinsom_is_admin($user_id)){
$data_arr['code']=0;
$data_arr['msg']='对方开启了防骚扰，只允许他关注的用户给他发送消息';
header('content-type:application/json');
echo json_encode($data_arr);
exit();		
}
...
if(!jinsom_is_admin($author_id)){//发送的非管理员
...
$status=$wpdb->query( "INSERT INTO $table_name (user_id,from_id,msg_content,msg_date,status) VALUES ('$author_id','$user_id','$content','$time','0')" );
if($status){
...
//如果开启了聊天隐私，并且没有关注对方，需要提醒：
if(get_user_meta($user_id,'im_privacy',true)&&!jinsom_is_follow_author($author_id,$user_id)){
$data_arr['im_privacy']=1;
$data_arr['im_privacy_tips']='由于你开启了聊天免扰，请先关注对方，否则对方无法联系你';
}
