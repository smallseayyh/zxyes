...
$client_id=$jinsom_social_login_tab['jinsom_login_wechat_code_key'];
$client_secret=$jinsom_social_login_tab['jinsom_login_wechat_code_secret'];
global $wpdb;

if(isset($_GET['code'])){
$code=$_GET['code'];
$response=wp_remote_get('https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$client_id.'&secret='.$client_secret.'&code='.$code.'&grant_type=authorization_code&redirect_uri='.home_url('/Extend/oauth/wechat/code/index.php'));
...
$appid_data=$wpdb->get_results("SELECT user_id FROM $wpdb->usermeta WHERE meta_key='weixin_pc_uid' AND meta_value='$openid' limit 1");
...
update_user_meta($user_id,"weixin_pc_uid",$openid);
...
setcookie('reg_type','wechat_code',time()+300,'/');
